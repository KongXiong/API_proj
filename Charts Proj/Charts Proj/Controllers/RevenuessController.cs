...
namespace Charts_Proj.Controllers
{
    public class RevenuesController : Controller
    {
        ...
        {
            var revenues = db.Revenues.Include(r => r.Client).Include(r => r.RevenueCategory);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,Total,Date,RegisteredUserID,ClientID,RevenueCategoryID")] Revenue revenue)
        {
            if (ModelState.IsValid)
            {
